...
const COOKIE_LOGIN_PATH = path.join(__dirname, "cookie_login.txt");
const COOKIE_STORAGE_PATH = path.join(__dirname, "cookie.txt");
...
========================`;
  ...
  // NEW: Cookie Management Functions
  static loadCookieLoginFile() {
    if (!fs.existsSync(COOKIE_LOGIN_PATH)) {
      return [];
    }
    try {
      const content = fs.readFileSync(COOKIE_LOGIN_PATH, 'utf8');
      ...
      lines.forEach((line, index) => {
        ...
        if (parts.length >= 3) {
          ...
          const cookie = parts.slice(2).join(':').trim(); // In case cookie contains colons
          cookies.push({ username, password, cookie, lineNumber: index + 1 });
        }
      });
      
      return cookies;
    } catch (e) {
      console.error(`❌ Lỗi khi đọc file cookie_login.txt: ${e.message}`);
      return [];
    }
  }

  static loadCookieStorageFile() {
    if (!fs.existsSync(COOKIE_STORAGE_PATH)) {
      return [];
    }
    try {
      const content = fs.readFileSync(COOKIE_STORAGE_PATH, 'utf8');
      const lines = content.split('\n').filter(line => line.trim() && !line.startsWith('#'));
      ...
      console.error(`❌ Lỗi khi đọc file cookie.txt: ${e.message}`);
      return [];
    }
  }

  static saveCookieToStorageFile(username, password, cookie) {
    try {
      const line = `${username}:${password}:${cookie}\n`;
      fs.appendFileSync(COOKIE_STORAGE_PATH, line);
      console.log(`✅ Đã lưu cookie vào file cookie.txt`);
      return true;
    } catch (e) {
      console.error(`❌ Lỗi khi lưu cookie: ${e.message}`);
      return false;
    }
  }

  static async checkCookieStatus(cookie) {
    try {
...
class UIRenderer {
  ...
  static renderTitle() {
    const fallbackTitle = `
╔════════════════════════════════════════════════════════════╗
║                   🛍️ STORE1ST.COM REJOIN 🛍️                 ║
╠════════════════════════════════════════════════════════════╣
║                                                            ║
║   📦 Bản quyền thuộc về STORE1ST.COM 📦                   ║
...
╚════════════════════════════════════════════════════════════╝`;
  ...
  // NEW: Display cookie status table
  static displayCookieStatus(cookies) {
    const table = new Table({
      head: ["STT", "Username", "Status", "User ID", "Display Name"],
      colWidths: [5, 20, 15, 15, 20],
      style: {
        head: ["cyan"],
        border: ["gray"]
      }
    ...
    cookies.forEach((cookie, index) => {
      const status = cookie.status.valid ? "✅ Live" : "❌ Die";
      const username = Utils.maskSensitiveInfo(cookie.username);
      const userId = cookie.status.userId ? Utils.maskSensitiveInfo(cookie.status.userId.toString()) : "N/A";
      const displayName = cookie.status.displayName ? Utils.maskSensitiveInfo(cookie.status.displayName) : "N/A";
...
╚════════════════════════════════════════════════════════════╝`);
      ...
      if (visitCount) {
        ...
        console.log(`SHOP store1st.com uy tín nhất VIỆT NAM 🇻🇳`);
      }
      console.log("\n🎯 Rejoin Tool");
      ...
      console.log("7. 🍪 Đăng nhập bằng Cookie (từ file cookie_login.txt)");
      console.log("8. 🍪 Lưu Cookie từ tài khoản đã đăng nhập");
      console.log("9. 🔍 Kiểm tra trạng thái Cookie");
      ...
      try {
        if (choice.trim() === "1") {
          ...
          await this.cookieLogin(rl);
          rl.close();
        } else if (choice.trim() === "8") {
          await this.saveCookiesFromAccounts(rl);
          rl.close();
        } else if (choice.trim() === "9") {
          await this.checkCookieStatus(rl);
          rl.close();
        } else {
  ...
  // NEW: Cookie Login Function
  async cookieLogin(rl) {
    console.clear();
    console.log(UIRenderer.renderTitle());
    console.log("\n🍪 Đăng nhập bằng Cookie từ file cookie_login.txt");

    const cookies = Utils.loadCookieLoginFile();
    
    if (cookies.length === 0) {
      console.log("❌ Không tìm thấy cookie nào trong file cookie_login.txt!");
      console.log("📝 Định dạng file: username:password:cookie");
      console.log("📁 File path: " + COOKIE_LOGIN_PATH);
      await new Promise(resolve => setTimeout(resolve, 3000));
      await this.start();
      return;
    }

    console.log(`\n📋 Tìm thấy ${cookies.length} cookie trong file:`);
    
    cookies.forEach((cookie, index) => {
      const maskedUsername = Utils.maskSensitiveInfo(cookie.username);
      console.log(`${index + 1}. ${maskedUsername} (Dòng ${cookie.lineNumber})`);
    });

    console.log("\n0. 🔄 Quay lại menu chính");

    const choice = await Utils.ask(rl, "\nChọn cookie để đăng nhập (0 để hủy): ");
    ...
    if (isNaN(index) || index < 0 || index >= cookies.length) {
      console.log("❌ Lựa chọn không hợp lệ!");
      await new Promise(resolve => setTimeout(resolve, 2000));
      await this.cookieLogin(rl);
      return;
    ...
    const selectedCookie = cookies[index];
    console.log(`\n🔍 Đang kiểm tra cookie cho ${Utils.maskSensitiveInfo(selectedCookie.username)}...`);

    const status = await Utils.checkCookieStatus(selectedCookie.cookie);
    
    if (status.valid) {
      console.log(`✅ Cookie hợp lệ!`);
      console.log(`👤 Username: ${status.username}`);
      console.log(`🆔 User ID: ${status.userId}`);
      console.log(`📛 Display Name: ${status.displayName || 'N/A'}`);
      
      // Ở đây có thể thêm logic để sử dụng cookie này cho các tác vụ khác
      console.log("\n💡 Cookie đã sẵn sàng để sử dụng!");
    } else {
      console.log(`❌ Cookie không hợp lệ hoặc đã hết hạn!`);
    }
  ...
  // NEW: Save Cookies from Logged-in Accounts
  async saveCookiesFromAccounts(rl) {
    console.clear();
    console.log(UIRenderer.renderTitle());
    console.log("\n🍪 Lưu Cookie từ tài khoản đã đăng nhập");

    console.log("🔍 Đang quét packages Roblox...");
    ...
    console.log(`\n📦 Tìm thấy ${Object.keys(packages).length} package Roblox:`);
    Object.values(packages).forEach((pkg, index) => {
      console.log(`${index + 1}. ${pkg.displayName} (${pkg.packageName})`);
    });

    const choice = await Utils.ask(rl, "\nChọn package để lấy cookie (0 để lấy tất cả): ");
    let selectedPackages = [];

    if (choice.trim() === "0") {
      selectedPackages = Object.keys(packages);
      console.log("🚀 Sẽ lấy cookie từ tất cả packages!");
    } else {
      const index = parseInt(choice) - 1;
      if (isNaN(index) || index < 0 || index >= Object.keys(packages).length) {
        console.log("❌ Lựa chọn không hợp lệ!");
        await new Promise(resolve => setTimeout(resolve, 2000));
        await this.saveCookiesFromAccounts(rl);
        return;
      }
      selectedPackages = [Object.keys(packages)[index]];
      console.log(`🎯 Sẽ lấy cookie từ: ${packages[selectedPackages[0]].displayName}`);
    }

    let savedCount = 0;
    for (const packageName of selectedPackages) {
      console.log(`\n🔍 Đang lấy cookie từ ${packages[packageName].displayName}...`);
      
      const cookie = Utils.getRobloxCookie(packageName);
      if (!cookie) {
        console.log(`❌ Không thể lấy cookie từ ${packageName}`);
        continue;
      }

      // Tạo user để lấy thông tin
      const user = new RobloxUser(null, null, cookie);
      const userId = await user.fetchAuthenticatedUser();
      
      if (!userId) {
        console.log(`❌ Cookie từ ${packageName} không hợp lệ!`);
        continue;
      }

      // Lưu cookie vào file
      const success = Utils.saveCookieToStorageFile(
        user.username || 'unknown',
        'unknown', // Password không thể lấy được
        cookie
      );

      if (success) {
        console.log(`✅ Đã lưu cookie cho ${Utils.maskSensitiveInfo(user.username)}`);
        savedCount++;
      }
    }

    console.log(`\n📊 Đã lưu thành công ${savedCount}/${selectedPackages.length} cookie!`);
    console.log(`📁 File lưu: ${COOKIE_STORAGE_PATH}`);
    
    await new Promise(resolve => setTimeout(resolve, 3000));
    await this.start();
  }

  // NEW: Check Cookie Status
  async checkCookieStatus(rl) {
    console.clear();
    console.log(UIRenderer.renderTitle());
    console.log("\n🔍 Kiểm tra trạng thái Cookie từ file cookie.txt");

    const cookies = Utils.loadCookieStorageFile();
    
    if (cookies.length === 0) {
      console.log("❌ Không tìm thấy cookie nào trong file cookie.txt!");
      console.log("📝 Định dạng file: username:password:cookie");
      console.log("📁 File path: " + COOKIE_STORAGE_PATH);
      await new Promise(resolve => setTimeout(resolve, 3000));
      await this.start();
      return;
    }

    console.log(`\n🔍 Đang kiểm tra ${cookies.length} cookie...`);
    console.log("⏳ Vui lòng chờ...");

    // Kiểm tra từng cookie
    const checkedCookies = [];
    for (const cookie of cookies) {
      const status = await Utils.checkCookieStatus(cookie.cookie);
      checkedCookies.push({
        ...cookie,
        status
      });
    ...
    // Thống kê
    const liveCookies = checkedCookies.filter(c => c.status.valid);
    const deadCookies = checkedCookies.filter(c => !c.status.valid);

    console.clear();
    console.log(UIRenderer.renderTitle());
    console.log("\n📊 KẾT QUẢ KIỂM TRA COOKIE");
    console.log("=" .repeat(50));
    console.log(`✅ Cookie sống: ${liveCookies.length}`);
    console.log(`❌ Cookie chết: ${deadCookies.length}`);
    console.log(`📊 Tổng số: ${checkedCookies.length}`);
    console.log("=" .repeat(50));

    // Hiển thị bảng chi tiết
    console.log("\n📋 CHI TIẾT COOKIE:");
    console.log(UIRenderer.displayCookieStatus(checkedCookies));

    // Gợi ý hành động
    if (deadCookies.length > 0) {
      console.log(`\n⚠️ Có ${deadCookies.length} cookie đã chết!`);
      console.log("💡 Bạn nên xóa các cookie chết khỏi file cookie.txt");
    }
